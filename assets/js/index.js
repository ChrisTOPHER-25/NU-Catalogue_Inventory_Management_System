$(document).ready(function() {
	...
	$("body").on('click', '.mark-paid', function(event) {
		var id = $(this).data('id');
		var status = $(this).data('status');
		var message = status === 'paid' ? 'pending' : 'paid';
		const confirm = window.confirm(`Are you sure you want to mark this sales as ${message}`);

		if(confirm) {
			window.location.href = base_url + `/SalesController/togglePaymentStatus?id=${id}&status=${message}`;
		}
	});
	...
	$("#sales-customer-id").change(function() {
		var id = $(this).val();
		sales_table.columns(2).search(id).draw();
	});
		...
		var id = $(this).data('id');
		 ...
		 $.ajax({
			...
			success : function(data) {
				...
				$("#customer-edit input[name='studentNumber']").val(customer.studentNumber);
				$("#customer-edit input[name='yearLevel']").val(customer.yearLevel);
				$("#customer-edit input[name='itemPurchased']").val(customer.itemPurchased);
				$("#customer-edit input[name='itemSize']").val(customer.itemSize);
				$("#customer-edit input[name='quantity']").val(customer.quantity);  
			}
